...
public class Example {
	...
	// Rather than keeping the name of the feature (i.e. the header
	// information), we'll
	// index the features starting at 0. The DataSet class will keep a mapping
	// from
	...
	private double weight; // Used for ensemble learning
	 ...
	 * 
	 * @param featureNum
	 *            the index of the feature to be added
	 * @param value
	...
	public double getFeature(int featureNum) {
		return sparseData.containsKey(featureNum) ? sparseData.get(featureNum)
				: 0.0;
	}
	...
	public String toString() {
		...
		for (int featureIndex : indices) {
			buffer.append(" " + featureIndex + ":"
					+ valueToString(sparseData.get(featureIndex)));
		}
	...
	public String toString(HashMap<Integer, String> featureMap) {
		...
		for (int featureIndex : indices) {
			buffer.append(" " + featureMap.get(featureIndex) + ":"
					+ valueToString(sparseData.get(featureIndex)));
		}
	...
	/**
	 * Helper function to output integers as integer strings instead of as
	 * doubles
	 * 
	 * @param val
	 * @return
